                            ...
                            @foreach($dashboards as $title => $items)
                                <div class="card">
                                    ...
                                    <div id="collapseOne{{ str_slug($title) }}"
                                         ...
                                         aria-labelledby="headingOne{{ str_slug($title) }}"
                                         data-parent="#accordionExample">
                                        <div class="card-body">
                                            <ul>
                                                @foreach($items as $item)
                                                    <li title="{{ $item['slug'] }}">
                                                        <input type="checkbox" name="dashboards[]"
                                                               value="{{ $item['slug'] }}" {{ isset($role->dashboard_access) && in_array($item['slug'], $role->dashboard_access->toArray()) ? 'checked' : '' }}>
                                                        <strong>{{ $item['name'] }}</strong>
                                                        <small
                                                            class="text-muted float-right">{{ $item['slug'] }}</small>

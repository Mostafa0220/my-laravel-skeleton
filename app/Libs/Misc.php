...
class Misc {
    ...
    public function uploadAndResizeMultiple($file, $object, $field,
           $imageSizes = []) {
           if (!$imageSizes) {
           ...
           $uploadPath = 'uploads/';
           if ($file) {
               $fileName = rand(111111, 99999999999999) . time() . '.' . $file->getClientOriginalExtension();
    ...
    public static function strip($text) {
        return strip_tags($text,
            '<br><p><a><b><u><i><ul><ol><li><table><tr><td><th><tbody><thead>');
    }
    ...
    public static function meta_keyword($text) {
        ...
        if ($arr) {
            foreach ($arr as $r) {
                $r = strtolower($r);
                if (!empty($r) and ! in_array($r, $words) and ! in_array($r,
                        $stopWords) and ! is_numeric($r)) {
                    $words[] = trim($r);
                }
            }
        }
        ...
        if ($value) {
            ...
            elseif ($type == "more_images") {
                if (File::exists("uploads/small/" . $value)) {
                    ...
                    $suffix.='<img class="cropped_preview" src="uploads/small/' . $value . '" width="80">';
                    //$suffix.='<p><a class="btn btn-danger" href="images/delete/' . $value . '" data-confirm="' . trans("user.Are you sure you want to delete this item") . '?' . '" data-title="' . trans('user.Confirmation message') . '"><i class="fa fa-trash-o"></i> ' . trans('user.Delete') . '</a></p>';
                    $suffix.='<a class="delete_image btn btn-danger" href=images/delete/' . $value . '><i class="fa fa-times"></i></a>';
                    //$suffix.='</p>';
                    $suffix.='</li>';
            ...
            elseif ($type == "file") {
                if (File::exists("uploads/" . $value)) {
                    ...
                    $suffix.='<a href="uploads/' . $value . '" class="btn btn-success">' . trans("user.Download") . '</a>';
                }
            }
            elseif ($type = "youtube") {
